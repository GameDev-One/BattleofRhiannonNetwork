...
# Thanks to neynt from https://github.com/godotengine/godot/issues/21137
...
const MAX_FOLDER_SIZE = 40

signal chip_item_selected(index)

export (Vector2) var delta_for_swipe := Vector2(8, 8)

onready var chip_items: VBoxContainer = $VBoxContainer
var chip_item_template = preload("res://src/ui/Menu/ChipItem.tscn")
...
func _ready():
	# Hide scrollbar
	get_v_scrollbar().modulate = Color(0, 0, 0, 0)
	
	for i in MAX_FOLDER_SIZE:
		var instance = chip_item_template.instance()
		$VBoxContainer.add_child(instance)
		instance.chip_name.text = ""
		instance.chip_damage.text = ""
		instance.chip_element.texture = null
		instance.chip_rank.text = ""
		instance.connect("scroll_button_pressed", self, "_on_scroll_button_pressed")
		
	chip_items.get_child(0).box_cursor.visible = true


func update_ui(fldr: BattlechipFolder):
	for i in MAX_FOLDER_SIZE:
		chip_items.get_children()[i].update_ui(fldr.battlechips[i])
...
func _input(ev) -> void:
	...
	if ev is InputEventMouseMotion:
		...
		if swiping:
			...
			var a = round(swipe_start.x - delta.x)
			var b = swipe_start.y - delta.y
			set_h_scroll(a)
			set_v_scroll(b)
			swipe_mouse_times.append(OS.get_ticks_msec())
			swipe_mouse_positions.append(ev.global_position)
...
func _on_scroll_button_pressed(index: int):
	for item in chip_items.get_children():
		item.box_cursor.visible = false
		
	emit_signal("chip_item_selected", index)
