...
func _process(delta):
	...
	for i in range(points.size()):
		...
		var currColor: Color = Color(1,1,1,1)
		if color_gradient:
			currColor = currColor * color_gradient.interpolate(1.0 - t)
		else:
			currColor = startColor.linear_interpolate(endColor, 1 - t)
		set_color(currColor)
...
func _interpolate_color() -> Color:
	var current_color = Color(1.0, 1.0, 1.0, 1.0)
	for i in range(1, points.size()):
		var factor :float = float(i) / (points.size() - 1)
		current_color = current_color * color_gradient.interpolate(1.0 - factor)
